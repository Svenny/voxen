...
template<typename T>
concept MessagePayload = std::is_void_v<T> || (std::is_trivially_copyable_v<T> &&
	sizeof(T) <= sizeof(MessagePayloadStore) && alignof(T) <= alignof(MessagePayloadStore));
...
public:
	...
	T &getPayload() noexcept { return *std::launder(reinterpret_cast<T *>(&m_payload)); }

	template<MessagePayload T>
	const T &getPayload() const noexcept { return *std::launder(reinterpret_cast<const T *>(&m_payload)); }
