...
template<typename T>
CompressedChunkStorage<T>::CompressedChunkStorage(const ExpandedArray &expanded)
{
	auto construct_node = [&](uint32_t index, Node &output) {
		...
		for (uint32_t i = 0; i < 64; i++) {
			...
			expanded.gather(leafBaseOffset(node_base, i), leaf_cube);
		...
		// Non-uniform node, allocate leaves.
...
CompressedChunkStorage<bool>::CompressedChunkStorage(const ExpandedArray &expanded)
{
	Node nodes[64];
	uint32_t used_nodes = 0;
	...
	for (uint32_t i = 0; i < 64; i++) {
		...
		expanded.gather(nodeBaseOffset(i), node_bools);
...
void CompressedChunkStorage<bool>::expand(ExpandedArray &expanded) const noexcept
{
	const Node *node = m_nodes.get();
...
template class CompressedChunkStorage<uint8_t>;
template class CompressedChunkStorage<uint16_t>;
template class CompressedChunkStorage<uint32_t>;
