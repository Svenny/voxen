...
void resolveEdgeEntry(std::span<const PointAccumulatorEntry> points, EdgeEntry &output)
{
	glm::vec3 pivot_normal(0.0f, 0.0f, 0.0f);
	pivot_normal[output.axis] = output.lower_endpoint_solid ? 1.0f : -1.0f;

	// Small weight to make the sum never resolve to zero
	const float INITIAL_WEIGHT = 0.05f;

	float weighted_offset_sum = 0.5f * INITIAL_WEIGHT;
	glm::vec3 weighted_normal_sum = pivot_normal * INITIAL_WEIGHT;
	float weight_sum = INITIAL_WEIGHT;

	for (const PointAccumulatorEntry entry : points) {
		float offset = entry.cell_base_offset[output.axis];
		float weight = std::max(0.0f, glm::dot(pivot_normal, entry.normal));

		weighted_offset_sum += offset * weight;
		weighted_normal_sum += entry.normal * weight;
		weight_sum += weight;
	}

	float resolved_offset = weighted_offset_sum / weight_sum;
	glm::vec3 resolved_normal = glm::normalize(weighted_normal_sum / weight_sum);

	output.offset_unorm = static_cast<uint16_t>(resolved_offset * 65535.0f);
	output.normal_y_sign = resolved_normal.y < 0.0f ? 1 : 0;
	output.normal_x_snorm = static_cast<int16_t>(resolved_normal.x * 32767.0f);
	output.normal_z_snorm = static_cast<int16_t>(resolved_normal.z * 32767.0f);
}
	...
	extras::dyn_array<EdgeEntry> &edge_entries, extras::dyn_array<CellEntry> &cell_entries)
{
	...
	for (uint32_t cell = 0; cell < BLOCKS * BLOCKS * BLOCKS; cell++) {
		...
		if (cell_accum.base_corner_sign != cell_accum.x_corner_sign) {
			EdgeEntry &entry = edge_entries[next_edge_entry];
			entry = {};
			entry.x = x;
			entry.y = y;
			entry.z = z;
			entry.axis = 0;
			entry.lower_endpoint_solid = cell_accum.base_corner_sign;

			resolveEdgeEntry(cell_accum.points, entry);
			next_edge_entry++;
		}

		if (cell_accum.base_corner_sign != cell_accum.y_corner_sign) {
			EdgeEntry &entry = edge_entries[next_edge_entry];
			entry = {};
			entry.x = x;
			entry.y = y;
		...
		if (cell_accum.base_corner_sign != cell_accum.z_corner_sign) {
			EdgeEntry &entry = edge_entries[next_edge_entry];
			entry = {};
			entry.x = x;
			entry.y = y;
