...
int main()
{
	...
	FT_Face ft_face;
	error = FT_New_Face(ft_library, "/opt/qt-creator/share/qtcreator/fonts/SourceCodePro-Regular.ttf", 0, &ft_face);
	if (error != FT_Err_Ok) {
	...
	// Align up to powers of two so that mipmapping does not mix up values from different glyps
	while (max_width & (max_width - 1u)) {
		max_width++;
	}
	while (max_height & (max_height - 1u)) {
		max_height++;
	}

	printf("Glyph size after power-of-two adjustment: %ux%u\n", max_width, max_height);
	...
	bool result = voxen::FileManager::writeUserFile("/run/user/1000/voxen/font.png", packed_png, true);
	if (!result) {
	...
	result = voxen::FileManager::writeUserFile("/run/user/1000/voxen/font.inc", std::as_bytes(descr_span), true);
	if (!result) {
